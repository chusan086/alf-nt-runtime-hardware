...
module n12_alf_key_tweak (
    ...
    input  logic         descriptor_capture_i,
    ...
    input  logic [127:0] ticket_q_minus_one_i,
    input  logic [3:0]   ticket_n_i,
    input  logic [4:0]   ticket_rounds_i,
    input  logic         ticket_identity_i,
    input  logic [2:0]   ticket_branch_count_i,
    input  logic [34:0]  ticket_branch_valid_i,
    input  logic [24:0]  ticket_branch_emit_i,
    input  logic [19:0]  ticket_branch_spill_i,
    input  logic [34:0]  ticket_branch_source_limit_i,
    ...
    logic         ticket_valid_q;
    logic         ticket_identity_q;
    logic         ticket_cold_q;
    logic         ticket_decrypt_q;
    logic [127:0] ticket_key_q;
    logic [63:0]  ticket_app_id_q;
    logic [127:0] ticket_tweak_q;
    logic [127:0] ticket_q_minus_one_q;
    logic [3:0]   ticket_n_q;
    logic [4:0]   ticket_rounds_q;
    logic [2:0]   ticket_branch_count_q;
    logic [34:0]  ticket_branch_valid_q;
    logic [24:0]  ticket_branch_emit_q;
    logic [19:0]  ticket_branch_spill_q;
    logic [34:0]  ticket_branch_source_limit_q;
    ...
    logic [3:0] branch_spill_after;
    logic [6:0] branch_source_limit;
    ...
    always_comb begin
        branch_count = {13'd0, ticket_branch_count_q};
        branch_valid_bytes = 7'd0;
        branch_emit_count = 7'd0;
        branch_spill_after = 4'd0;
        branch_source_limit = 7'd0;
        case (branch_index_q)
            3'd0: begin
                branch_valid_bytes = ticket_branch_valid_q[0 +: 7];
                branch_emit_count = {2'd0, ticket_branch_emit_q[0 +: 5]};
                branch_spill_after = ticket_branch_spill_q[0 +: 4];
                branch_source_limit = ticket_branch_source_limit_q[0 +: 7];
            end
            3'd1: begin
                branch_valid_bytes = ticket_branch_valid_q[7 +: 7];
                branch_emit_count = {2'd0, ticket_branch_emit_q[5 +: 5]};
                branch_spill_after = ticket_branch_spill_q[4 +: 4];
                branch_source_limit = ticket_branch_source_limit_q[7 +: 7];
            end
            3'd2: begin
                branch_valid_bytes = ticket_branch_valid_q[14 +: 7];
                branch_emit_count = {2'd0, ticket_branch_emit_q[10 +: 5]};
                branch_spill_after = ticket_branch_spill_q[8 +: 4];
                branch_source_limit = ticket_branch_source_limit_q[14 +: 7];
            end
            3'd3: begin
                branch_valid_bytes = ticket_branch_valid_q[21 +: 7];
                branch_emit_count = {2'd0, ticket_branch_emit_q[15 +: 5]};
                branch_spill_after = ticket_branch_spill_q[12 +: 4];
                branch_source_limit = ticket_branch_source_limit_q[21 +: 7];
            end
            3'd4: begin
                branch_valid_bytes = ticket_branch_valid_q[28 +: 7];
                branch_emit_count = {2'd0, ticket_branch_emit_q[20 +: 5]};
                branch_spill_after = ticket_branch_spill_q[16 +: 4];
                branch_source_limit = ticket_branch_source_limit_q[28 +: 7];
            end
            default: begin
                branch_valid_bytes = 7'd0;
                branch_emit_count = 7'd0;
        ...
        emit_source_next = {1'b0, branch_source_q} + {3'd0, ticket_n_q} -
                           emit_spill_count;
        emit_leftover = {3'd0, branch_spill_after};
        ...
        prepare_pre_mix = prepare_input(current_key_o, ticket_n_q);
        ...
        if (local_run_permit && state_q == ST_KEY_INIT) begin
            ...
            aes_key_o = smac_message;
        end else if (local_run_permit && state_q == ST_PREPARE) begin
            aes_active_o = 1'b1;
            aes_mode_o = 3'd4;
    ...
    // The only address arithmetic is a local increment/decrement already
    // implied by the phase counters.  The final key-emission edge primes enc0;
    // PREPARE then reads enc[i+1] while AES consumes the held enc[i].
    always_comb begin
        key_bank_write_enable = 1'b0;
        key_bank_write_bank = 1'b0;
        key_bank_write_index = 5'd0;
        key_bank_write_data = 128'h0;
        if (local_run_permit && state_q == ST_TWEAK_INIT && emit_active_q) begin
            key_bank_write_enable = 1'b1;
            key_bank_write_bank = 1'b0;
            key_bank_write_index = key_write_index_q;
            key_bank_write_data = extracted_key;
        end else if (local_run_permit && state_q == ST_PREPARE &&
                     aes_outstanding_q && aes_result_valid_i) begin
            key_bank_write_enable = 1'b1;
        ...
        if (state_q == ST_PREPARE && aes_outstanding_q &&
            aes_result_valid_i && phase_cycle_q + 1'b1 < ticket_rounds_q) begin
            ...
            key_bank_read_index = prepare_index + 1'b1;
                     ...
                     (branch_final ||
                      key_write_index_q + 1'b1 >= ticket_rounds_q)) begin
            key_bank_read_enable = 1'b1;
            key_bank_read_bank = 1'b0;
    ...
    assign local_identity_match_o = ticket_valid_q && expected_valid_i &&
                                    (ticket_identity_q == expected_identity_i);
    assign local_run_permit = local_identity_match_o && execution_permit_i;
    assign descriptor_ready_o = (state_q == ST_IDLE);
    assign kt_start_ready_o = (state_q == ST_IDLE) && local_run_permit;
    ...
    assign tweak_complete_now = (state_q == ST_TWEAK_INIT) &&
                                ...
                                (branch_final ||
                                 key_write_index_q + 1'b1 >= ticket_rounds_q);
    assign prepare_complete_now = (state_q == ST_PREPARE) &&
                                  aes_outstanding_q && aes_result_valid_i &&
                                  (phase_cycle_q + 1'b1 >= ticket_rounds_q);
    assign kt_done_valid_o = local_run_permit &&
                             ((state_q == ST_DONE) ||
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            ticket_valid_q <= 1'b0;
            ticket_identity_q <= 1'b0;
            ticket_cold_q <= 1'b0;
            ticket_decrypt_q <= 1'b0;
            ticket_key_q <= 128'h0;
            ticket_app_id_q <= 64'h0;
            ticket_tweak_q <= 128'h0;
            ticket_q_minus_one_q <= 128'h0;
            ticket_n_q <= 4'd0;
            ticket_rounds_q <= 5'd0;
            ticket_branch_count_q <= 3'd0;
            ticket_branch_valid_q <= 35'd0;
            ticket_branch_emit_q <= 25'd0;
            ticket_branch_spill_q <= 20'd0;
            ticket_branch_source_limit_q <= 35'd0;
            work_a1_q <= 128'h0;
            work_a2_q <= 128'h0;
            work_a3_q <= 128'h0;
            ...
            if (descriptor_capture_i && descriptor_ready_o) begin
                ticket_valid_q <= 1'b1;
                ticket_identity_q <= ticket_identity_i;
                ticket_cold_q <= ticket_cold_i;
                ticket_decrypt_q <= ticket_decrypt_i;
                ticket_key_q <= ticket_key_i;
                ticket_app_id_q <= ticket_app_id_i;
                ticket_tweak_q <= ticket_tweak_i;
                ticket_q_minus_one_q <= ticket_q_minus_one_i;
                ticket_n_q <= ticket_n_i;
                ticket_rounds_q <= ticket_rounds_i;
                ticket_branch_count_q <= ticket_branch_count_i;
                ticket_branch_valid_q <= ticket_branch_valid_i;
                ticket_branch_emit_q <= ticket_branch_emit_i;
                ticket_branch_spill_q <= ticket_branch_spill_i;
                ticket_branch_source_limit_q <=
                    ticket_branch_source_limit_i;
            end
            if (local_run_permit) begin
              case (state_q)
                ST_IDLE: begin
                    phase_cycle_q <= 16'd0;
                    aes_outstanding_q <= 1'b0;
                    if (kt_start_valid_i && kt_start_ready_o) begin
                        ...
                        state_q <= ticket_cold_q ? ST_ENGINE_INIT : ST_TWEAK_INIT;
                    end
                end
                                ...
                                for (integer lane = 0; lane < 16; lane = lane + 1) begin
                                    if (lane < emit_leftover)
                                        key_spill_q[8*lane +: 8] <= branch_byte(
                                            branch_result,
                                            integer'(branch_source_limit) + lane);
                                end
                                spill_valid_q <= branch_spill_after;
                ...
                ST_PREPARE: begin
                    ...
                    if (aes_outstanding_q && aes_result_valid_i) begin
                        aes_outstanding_q <= 1'b0;
                        if (phase_cycle_q + 1'b1 >= ticket_rounds_q) begin
                            phase_cycle_q <= 16'd0;
                            state_q <= kt_done_ready_i ? ST_IDLE : ST_DONE;
                        end else begin
                            phase_cycle_q <= phase_cycle_q + 1'b1;
