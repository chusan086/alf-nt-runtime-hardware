...
module n12_alf_round_core (
    ...
    input  logic         descriptor_load_i,
    input  logic         descriptor_identity_i,
    input  logic         expected_valid_i,
    input  logic         expected_identity_i,
    input  logic         transaction_retire_i,
    input  logic         start_i,
    input  logic         descriptor_decrypt_i,
    input  logic [127:0] descriptor_q_i,
    input  logic [3:0]   descriptor_n_i,
    input  logic [2:0]   descriptor_t_i,
    input  logic [4:0]   descriptor_rounds_i,
    input  logic [4:0]   descriptor_rounds_per_segment_i,
    input  logic [4:0]   descriptor_segment_count_i,
    input  logic [127:0] descriptor_input_i,
    ...
    logic decrypt_q;
    logic [127:0] ticket_q_q;
    logic [3:0] ticket_n_q;
    logic [2:0] ticket_t_q;
    logic [4:0] ticket_rounds_q;
    logic [4:0] ticket_rounds_per_segment_q;
    logic [4:0] ticket_segment_count_q;
    logic [127:0] ticket_input_q;
    logic [3:0] n_q;
    logic [2:0] t_q;
    logic [4:0] k_q;
    ...
    assign candidate_accepted = candidate_q < ticket_q_q;
    ...
    always_comb begin
        use_n = start_i ? ticket_n_q : n_q;
        use_t = start_i ? ticket_t_q : t_q;
        base_x = start_i ? split_x(ticket_input_q, ticket_n_q) : x_q;
        base_e = start_i ? split_e(ticket_input_q, ticket_n_q, ticket_t_q) : e_q;

        key_required = 1'b0;
        expected_key_bank = 1'b0;
        expected_key_index = 5'd0;
        aes_active_o = active_i && local_identity_match_o &&
                       !candidate_pending_q;
        aes_decrypt_o = 1'b0;
        aes_last_o = 1'b0;
        aes_state_o = 128'h0;
        ...
        if (start_i) begin
            ...
            case (state_q)
                ST_ENC_ROUND: begin
                    ...
                    aes_state_o = shuffle_profile(x_q, n_q, 3'd3);
                    aes_key_o = current_key_i;
                end
                ST_DEC_AUX: begin
                    aes_state_o = shuffle_profile(x_q, n_q, 3'd3);
                ...
                ST_DEC_ROUND: begin
                    ...
                    expected_key_index = first_round_q + round_offset_q;
                    aes_state_o = shuffle_profile(x_q, n_q, 3'd6);
                    aes_key_o = current_key_i;
                    aes_decrypt_o = 1'b1;
                end
                default: begin
                    aes_state_o = shuffle_profile(x_q, n_q, 3'd7);
                    aes_decrypt_o = 1'b1;
                    aes_last_o = 1'b1;
                end
            endcase
        ...
        dec_next_e = update_e(
            ...
            (n_q == 3 || n_q == 7 || n_q == 11 || n_q == 15) ? 8'h52 : 8'h00,
            t_q
        );
        dec_next_x = aes_result_i ^
                     shuffle_profile(aes_result_i, n_q, 3'd5) ^
                     shuffle_profile(e_block(dec_next_e), n_q, 3'd2);

        candidate_value = 128'h0;
        if (!start_i && state_q == ST_ENC_ROUND && round_offset_q + 1'b1 == k_q) begin
            candidate_value = join_value(enc_next_x, enc_next_e, n_q);
        end else if (!start_i && state_q == ST_DEC_FINAL) begin
            candidate_value = join_value(aes_result_i, e_q, n_q);
        end
        attempt_done_o = active_i && local_identity_match_o &&
                         candidate_pending_q;
    ...
    // Prefetch is deliberately shallow: only local registered counters and
    // +/-1 address steps feed the RAM.  The fetched key is registered by the
    // key bank before it can reach the AES lane.
    always_comb begin
        key_prefetch_valid_o = 1'b0;
        key_prefetch_bank_o = 1'b0;
        key_prefetch_index_o = 5'd0;
        if (active_i && local_identity_match_o) begin
            if (candidate_pending_q && state_q == ST_ENC_ROUND &&
                ...
                if (start_i) begin
                    if (decrypt_q) begin
                        ...
                        key_prefetch_index_o = ticket_rounds_q - 1'b1;
                    end else if (ticket_rounds_per_segment_q > 1) begin
                        key_prefetch_valid_o = 1'b1;
...
`endif
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            ticket_rounds_per_segment_q <= 5'd0;
            ticket_segment_count_q <= 5'd0;
            ticket_input_q <= 128'h0;
            n_q <= 4'd0;
            t_q <= 3'd0;
            k_q <= 5'd0;
          ...
          if (descriptor_load_i && !candidate_pending_q) begin
                ticket_valid_q <= 1'b1;
                ticket_identity_q <= descriptor_identity_i;
                decrypt_q <= descriptor_decrypt_i;
                ticket_q_q <= descriptor_q_i;
                ticket_n_q <= descriptor_n_i;
                ticket_t_q <= descriptor_t_i;
                ticket_rounds_q <= descriptor_rounds_i;
                ticket_rounds_per_segment_q <=
                    descriptor_rounds_per_segment_i;
                ticket_segment_count_q <= descriptor_segment_count_i;
                ticket_input_q <= descriptor_input_i;
          end
          if (step_i) begin
            if (active_i && local_identity_match_o) begin
                if (candidate_pending_q) begin
                    ...
                    if (state_q == ST_ENC_ROUND) begin
                        ...
                        x_q <= split_x(candidate_q, n_q);
                        e_q <= split_e(candidate_q, n_q, t_q);
                        state_q <= ST_DEC_AUX;
                        if (!data_done_o) begin
                            if (candidate_accepted) begin
                                segment_q <= segment_q + 1'b1;
                    ...
                    n_q <= ticket_n_q;
                    t_q <= ticket_t_q;
                    k_q <= ticket_rounds_per_segment_q;
                    segment_count_q <= ticket_segment_count_q;
                    segment_q <= 5'd0;
                    attempt_q <= 16'd0;
                    first_round_q <= decrypt_q ?
                        (ticket_rounds_q - ticket_rounds_per_segment_q) : 5'd0;
                    if (decrypt_q) begin
                        x_q <= aes_result_i;
                        e_q <= split_e(ticket_input_q, ticket_n_q, ticket_t_q);
                        round_offset_q <= ticket_rounds_per_segment_q - 1'b1;
